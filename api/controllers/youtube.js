...
// Server-side proxy: tries Invidious instances so the browser never needs to
// connect to them directly (avoids CSP violations).
exports.ytPlaylistProxy = async (req, res, next) => {
  ...
  for (const inst of instances) {
    try {
      ...
      if (!upstream.ok) { lastErr = `${inst} → HTTP ${upstream.status}`; continue; }
